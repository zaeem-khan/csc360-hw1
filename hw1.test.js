...
test('validate Circle calcArea method', () => {
    ...
    expect(circle.calcArea()).toBe(circle.radius*circle.radius*Math.PI);
});
...
const products = [
    ...
    new Product('Beef,6.99,Out of Stock')
];
...
test('calls Product.printProducts() method, passing a list of products, asserts all products are printed matching specific formatting provided', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Product: Apple, Cost: $1.00, Availability: Yes')
    expect(console.log).toHaveBeenCalledWith('Product: Carrot, Cost: $0.50, Availability: No')
    expect(console.log).toHaveBeenCalledWith('Product: Bread, Cost: $3.00, Availability: Yes')
    expect(console.log).toHaveBeenCalledWith('Product: Beef, Cost: $6.99, Availability: No');
})